...
1. Navigate to "/" and see a basic splash page with the name of the website. - **COMPLETED**
2. See the site-wide header on every page with a link to home page with list of cities.  - **COMPLETED**

  - Create Site Layout component w/ router outlet to keep site uniform
...
5. View post "show" pages with title, author, and content.
  ...
  - import json from GeoDb api (on RapidAPI)
    - also achieves bonus objective: import api
...
3. A post's content must not be empty.
  ...
  - change api call to include template literal for city name on call
  - js limit input
  - html: required
